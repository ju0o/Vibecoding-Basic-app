...
2. **P-02 승인**: approved 상태 KB의 verification-report 검토 (게이트·점수 근거의 완전성) → 타당하면 qa_approved로 전이, 부실하면 RECOLLECT로 되돌리고 사유 기록
...
- 라우팅: 승인 완료 → 상태 기계가 가리키는 다음 RUN (보통 RUN-CODEX-PRODUCE) / 운영자 결정 대기 → Next: 운영자, If Approved·If Rejected에 각 경로 명시
```
