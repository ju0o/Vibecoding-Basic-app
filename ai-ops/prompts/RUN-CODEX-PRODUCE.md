...
Executor: **Codex 생산 단계** (검증 단계와 같은 흐름에서 연속 실행 가능)
...
4. planned 항목 중 근거 KB가 전부 approved 이상인 것 있음 → **P-04 강의 생성** (최대 4건/런)
...
5. **NEXT_ACTION 블록 작성** (규격: ai-ops/OPERATION_MANUAL.md의 "NEXT_ACTION 블록 규격") — 보고 맨 끝에 출력하고, 같은 내용을 STATE.md의 "## NEXT" 섹션에 덮어쓴다. 라우팅 규칙:
   - P-01 완료 → Next: **Codex / RUN-CODEX-VERIFY.md** (draft 발생, 같은 흐름에서 연속 실행 가능)
   - P-03 완료 → Next: **Codex / RUN-CODEX-VERIFY.md** (재평가, 같은 흐름에서 연속 실행 가능)
