...
1. P-01 (Codex 수집 세션) — KB 수집·생성          [개념 간 병렬 OK]
2. P-02 (Codex 검증 세션) — KB 검증·Score         [1번과 다른 세션 필수]
3. P-03 (Codex 수집 세션) — [Loop A] Score 미달 시에만 → P-02 재평가
4. Fable 승인            — P-02 보고서 검토 후 P-04 착수 허가 (프롬프트 없음)
5. P-04 (Codex)          — Lesson 생성            [slug 간 병렬 OK]
6. P-05 (Codex 단독 세션) — 사이트 반영            [반드시 단일 세션 순차]
7. P-06 (Cline)          — Build·Lint·Test·verify [판정만, 수정 금지]
8. P-07 (Codex)          — [Loop B] verify 실패 시에만 → P-06 재검증
9. P-08 (Cline)          — Release (src/content 포함 커밋, 배포는 운영자 승인)
10. O-02 (Fable)         — 강의 10개 릴리스마다 최종 편집 (매 배치 아님)
```
...
| 시간대 | 호출 | 내용 |
|---|---|---|
| 오전 | **Codex 수집 세션** (병렬 가능) | P-01 신규 수집 + P-03 어제 미달분 재수집 |
| 오전~오후 | **Codex 검증 세션** (수집과 분리) | P-02 검증 (수집 완료분부터) → **Fable 승인** → P-04 병렬 착수 |
| 저녁 (통합 창 1회) | **Codex 단독 세션** → **Cline** | P-05 순차 반영 → P-06 verify → (P-07 루프) → P-08 |
...
| 구간 | 규칙 |
|---|---|
| P-01, P-03 (지식 생산) | **개념 간 무제한 병렬** — Codex 다중 세션. 규칙 2개: 같은 개념의 P-02와 P-03 동시 실행 금지, **수집 세션과 검증 세션 분리** |
...
| Loop | 발동 조건 | 경로 | 상한 |
|---|---|---|---|
| **A. 지식 루프** | Knowledge Score < 80 또는 필수 게이트(무출처 주장 등) 실패 | P-02가 재수집 요청서 생성 → Codex 수집 세션 P-03 → P-02 재평가 | 2회. 3회째 미달 → `✗ escalated`, 운영자 판단 (주제 범위 재정의 등) |
