...
## 3.5 콜아웃 블록 (예시·강조 박스 — 2026-07-05 운영자 승인 추가)

가독성을 위한 박스형 강조. **작성자(Codex) 재량으로 배치** — 단 아래 상한 준수.

| 종류 | 문법 | 용도 |
|---|---|---|
| 예시 | `> [!EXAMPLE]` 블록 | 실전 예·시나리오를 본문과 시각 분리 |
| 핵심 | `> [!KEY]` | 섹션의 결론·판단 기준 요약 |
| 주의 | `> [!WARNING]` | 흔한 실수·위험 |
| 팁 | `> [!TIP]` | 실무 요령 |

- 렌더링: 종류별 색·아이콘 구분된 박스 (다크모드 대응) — 플랫폼 증분 D-02
- 남용 방지: **섹션당 최대 2개, 강의당 최대 8개.** 콜아웃이 본문을 대체하면 안 됨 (본문이 주, 박스는 보조)
- 인용 blockquote와 시각적으로 구분될 것 (인용 = 좌측 보더+출처 줄, 콜아웃 = 색 배경+아이콘)
