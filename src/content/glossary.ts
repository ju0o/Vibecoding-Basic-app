...
export const GLOSSARY_TERMS = [
  ...
  },
  {
    ...
    category: "AI 코딩 도구",
    shortDefinition:
      "AI에게 맡길 작업의 목표, 범위, 성공 기준, 금지 행동을 사람이 먼저 정리하는 단계",
    explanation:
      "Task Framing은 AI 협업의 출발점입니다. 목표가 흐리면 AI는 범위와 성공 기준을 추측하므로, 사람이 먼저 무엇을 바꾸고 무엇을 바꾸지 않을지, 어떤 검증을 통과해야 하는지 정해야 합니다.",
  ...
  },
  {
    ...
    category: "AI 코딩 도구",
    shortDefinition:
      "AI output을 사람이 diff, test, source check로 검토하고 그 feedback을 다음 prompt나 task로 되돌리는 반복 구조",
    explanation:
      "Human Review Loop는 AI output을 한 번에 채택하지 않고 검토 결과를 다시 작업 입력으로 보내는 협업 방식입니다. 실패한 test, review comment, 바뀐 요구사항을 다음 AI instruction으로 돌려 품질을 높입니다.",
    related: ["Review Workflow", "Code Review Boundary", "Verification"],
  },
  {
    term: "Responsibility Boundary",
    category: "AI 코딩 도구",
    shortDefinition:
      "AI가 만든 결과라도 repository에 들어가는 순간 사람과 팀이 책임진다는 검토 경계",
    explanation:
      "Responsibility Boundary는 AI가 작성했다는 이유로 책임이 분리되지 않는다는 원칙입니다. 코드가 commit, PR, release로 들어가기 전 사람이 output을 review하고 검증해야 합니다.",
    related: ["Code Review Boundary", "Verification", "Review Workflow"],
  },
  {
    term: "Rollback Readiness",
    category: "AI 코딩 도구",
    shortDefinition:
      "AI와 함께 만든 변경이 실패했을 때 되돌릴 수 있도록 diff, commit, test 결과를 작게 유지하는 준비 상태",
    explanation:
  ...
  },
  {
    ...
    explanation:
      "Test Matrix는 AI에게 테스트 초안을 만들게 하기 전에 정상 입력, 빈 값, 오류 조건, 권한, network failure 같은 scenario를 정리하는 단계입니다. 테스트 개수보다 어떤 behavior를 막는지가 중요합니다.",
  ...
  },
  {
    ...
    explanation:
      "User-like Test는 role, label, visible text, locator처럼 사용자-facing 계약을 기준으로 동작을 확인합니다. AI-generated test가 CSS class나 내부 state만 확인할 때 이 기준으로 보강할 수 있습니다.",
    related: ["Testing Library", "Playwright", "Frontend Test"],
  },
  {
    term: "Actionability Check",
    category: "AI 코딩 도구",
    shortDefinition:
      "Playwright가 action을 수행하기 전에 요소가 상호작용 가능한 상태인지 확인하는 검사",
    explanation:
      "Actionability Check는 manual sleep으로 기다리는 대신 locator가 실제로 보이고, 안정적이고, 사용 가능한지 확인하는 안정성 기준입니다. AI가 만든 E2E 테스트에서 flaky wait를 줄이는 데 중요합니다.",
    related: ["Playwright", "Locator", "Frontend Test"],
  },
  {
    term: "Repeatable Test Run",
    category: "AI 코딩 도구",
    shortDefinition:
      "watch mode 없이 한 번 실행되고 종료되어 CI와 검증 루프에서 반복 가능한 테스트 실행",
    explanation:
