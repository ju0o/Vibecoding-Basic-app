...
### 설명 연습: 조회 명령을 질문별로 고르기

git log, diff, show를 설명할 때는 명령 이름보다 질문을 먼저 놓으면 쉽습니다. "무슨 일이 있었나"를 묻는다면 log입니다. "지금 무엇이 달라졌나"를 묻는다면 diff입니다. "이 특정 커밋이나 객체가 무엇인가"를 묻는다면 show입니다. 세 명령은 모두 읽기 전용이지만, 답하는 질문이 다르므로 섞어 쓰면 판단이 흐려집니다.

diff를 볼 때는 비교 방향을 설명해야 합니다. 빼기 줄은 왼쪽 기준에 있었고 오른쪽 기준에는 없는 내용이며, 더하기 줄은 오른쪽 기준에 새로 생긴 내용입니다. 인자 순서를 바꾸면 이야기의 방향이 바뀝니다. 그래서 코드 리뷰에서는 "무엇을 기준으로 무엇을 비교하는가"를 먼저 확인해야 합니다.

AI 협업에서는 이 조회 명령들이 근거 수집 도구가 됩니다. 수정하기 전에 log로 관련 이력을 보고, diff로 현재 변경을 좁히고, show로 특정 커밋의 의도를 확인하면 AI에게 줄 context가 훨씬 정확해집니다. 좋은 수정은 좋은 조회에서 시작합니다.

### 스스로 점검할 질문

조회 명령을 실행하기 전에는 "나는 무엇을 알고 싶은가"를 먼저 정하세요. 최근 변경 흐름이 궁금하면 log를 봅니다. 아직 커밋하지 않은 수정이 궁금하면 diff를 봅니다. 특정 커밋 하나의 변경과 메시지가 궁금하면 show를 봅니다. 질문이 흐리면 출력이 길어지고, 출력이 길어지면 중요한 단서가 묻힙니다.

diff를 읽을 때는 파일명, hunk 위치, 삭제와 추가의 방향을 차례대로 봅니다. 색만 보고 좋고 나쁨을 판단하지 말고, 변경이 어떤 사용자 동작이나 테스트와 연결되는지 묻습니다. AI에게 리뷰를 시킬 때도 diff 전체를 던지기보다 "이 변경이 인증 흐름에 영향을 주는지", "이 삭제가 테스트 누락을 만들 수 있는지"처럼 질문을 좁히면 훨씬 쓸모 있는 답을 얻습니다.

show는 과거의 의도를 복원할 때 유용합니다. 현재 코드만 보면 이상해 보이는 구조도 특정 커밋의 메시지와 변경 범위를 보면 당시 문제를 해결하기 위한 선택이었을 수 있습니다. 기록을 읽는 능력은 과거의 나와 동료, AI가 만든 변경을 공정하게 판단하는 능력입니다.

마지막으로 조회 명령은 안전하지만 판단은 안전하지 않을 수 있습니다. 출력이 길면 일부만 보고 결론을 내리기 쉽고, 비교 방향을 착각하면 삭제와 추가를 반대로 읽습니다. 그래서 중요한 판단 전에는 같은 사실을 log, diff, show 중 두 가지 이상으로 교차 확인하는 습관이 좋습니다.

이 습관은 AI에게도 그대로 적용됩니다. AI가 요약한 diff가 맞는지 원문 diff와 커밋 메시지를 다시 대조하면, 그 다음 수정 지시가 훨씬 정확해집니다.

작게라도 직접 읽은 근거가 있으면 AI의 제안도 훨씬 안정적으로 평가할 수 있습니다.

조회는 수정의 준비 운동입니다. 준비가 좋을수록 실제 수정은 작고 안전해집니다.
