...
### 설명 연습: gh 명령을 PR 상태 전이로 읽기

GitHub CLI를 설명할 때는 명령을 외우기보다 PR의 상태가 어떻게 바뀌는지 따라가면 쉽습니다. create는 로컬 브랜치의 변경을 원격 협업 대상으로 올리고, list와 view는 열린 논의의 목록과 세부를 읽습니다. checkout은 남의 PR을 내 로컬 작업공간으로 가져와 실제 실행 검증을 가능하게 합니다. review는 comment, approve, request changes 중 하나로 사람의 판단을 기록합니다. merge는 그 판단이 끝난 변경을 base branch에 반영합니다.

이 관점은 AI 협업에서도 중요합니다. AI가 PR을 만들었다고 해서 검토가 끝난 것이 아니며, gh pr view와 checkout으로 diff와 실행 결과를 확인해야 합니다. 반대로 사람이 웹 UI에서 하던 반복 클릭을 CLI로 표준화하면 자동화와 기록이 쉬워집니다. 예를 들어 "PR 생성 → 테스트 통과 확인 → 리뷰 요청 → 승인 후 squash merge" 같은 팀 루틴을 명령 묶음으로 만들 수 있습니다.

주의할 점은 gh가 GitHub의 협업 상태를 바꾸는 도구라는 점입니다. 로컬 git 명령보다 조직 규칙, 권한, 브랜치 보호, 리뷰 요구 사항의 영향을 더 많이 받습니다. 명령이 실패하면 문법만 볼 것이 아니라 권한과 저장소 정책도 함께 봐야 합니다. CLI는 웹 UI의 짧은 길이지만, 협업 규칙을 건너뛰는 뒷문은 아닙니다.

### 스스로 점검할 질문

gh 명령을 쓰기 전에는 이 명령이 "로컬 저장소"를 바꾸는지 "GitHub의 협업 상태"를 바꾸는지 구분해 보세요. pr checkout은 로컬 브랜치를 만들 수 있지만, pr review는 원격 PR에 사람의 판단을 남깁니다. pr merge는 협업 결과를 base branch에 반영하므로 브랜치 보호와 승인 규칙을 통과해야 합니다.

AI에게 gh 명령을 맡길 때는 읽기 명령과 쓰기 명령을 분리하는 것이 안전합니다. 먼저 pr view, pr diff, checks 확인처럼 읽기 작업만 수행하게 하고, 그 결과가 충분할 때 review나 merge 같은 쓰기 작업으로 넘어갑니다. 이 순서를 지키면 자동화의 속도는 살리면서도 협업 기록을 안전하게 관리할 수 있습니다.

마지막으로 gh는 혼자 쓰는 도구가 아니라 팀 기록에 흔적을 남기는 도구입니다. 명령 하나가 리뷰 상태, 병합 상태, 브랜치 상태를 바꾸므로 실행 전후에 웹 UI나 pr view로 결과를 확인하는 습관이 필요합니다.

이 습관은 자동화가 커질수록 더 중요합니다. 빠른 명령일수록 작은 확인 문장을 남겨야 다음 사람이 같은 맥락에서 이어받을 수 있습니다.
