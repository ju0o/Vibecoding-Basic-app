...
### 설명 연습: scripts를 팀의 표준 버튼으로 말하기

npm scripts를 설명할 때는 "짧은 별명"보다 "팀이 합의한 표준 버튼"이라고 말하는 편이 좋습니다. 어떤 프로젝트는 빌드 명령이 next build이고, 어떤 프로젝트는 타입 생성이나 CSS 빌드가 먼저 필요합니다. 사용자가 모든 내부 명령을 외우지 않아도 npm run build 하나로 같은 절차를 실행하게 만드는 것이 scripts의 장점입니다.

이 표준 버튼은 CI/CD와 AI 협업에서도 중요합니다. 파이프라인은 보통 npm run lint, npm run typecheck, npm run test, npm run build처럼 scripts에 적힌 명령을 실행합니다. AI에게도 "검증해 줘"보다 "npm run verify를 실행하고 실패하면 첫 오류부터 고쳐라"라고 말하면 성공 기준이 분명해집니다. scripts는 문서이자 자동화 계약입니다.

다만 scripts는 결국 shell command입니다. 이름이 안전해 보여도 내부에서 파일 삭제, 배포, secret 출력 같은 작업을 할 수 있습니다. 그래서 낯선 프로젝트에서는 scripts를 먼저 읽고, 특히 postinstall 같은 자동 실행 훅을 확인해야 합니다. 편리함과 위험이 같은 문에 달려 있다는 점까지 설명해야 실무 감각이 붙습니다.

### 스스로 점검할 질문

새 프로젝트를 받으면 package.json의 scripts를 작은 운영 설명서처럼 읽어 보세요. 개발 서버는 어떤 명령으로 켜는지, 검증은 어떤 순서로 도는지, build가 어떤 산출물을 만드는지, deploy가 실제 배포까지 수행하는지 확인합니다. 이름이 같아도 프로젝트마다 내부 명령은 다를 수 있으므로, npm run build라는 표면만 보고 안심하면 안 됩니다.

AI에게 작업을 맡길 때도 scripts는 좋은 경계가 됩니다. "테스트해 줘"보다 "npm run verify를 실행하고 실패한 첫 단계부터 고쳐라"가 더 명확합니다. 반대로 "npm run deploy"처럼 외부 상태를 바꾸는 script는 승인과 대상 환경을 따로 요구해야 합니다. scripts는 자동화의 입구이므로, 어떤 입구가 읽기 전용이고 어떤 입구가 세상을 바꾸는지 구분하는 습관이 필요합니다.

마지막으로 scripts 이름은 팀의 언어입니다. 같은 명령 묶음을 모두가 같은 이름으로 부르면 문서, CI, AI 지시, 온보딩이 단순해집니다. 반대로 사람마다 다른 명령을 기억해야 하면 검증 누락과 배포 실수가 늘어납니다.

그래서 scripts를 바꿀 때는 코드 변경처럼 리뷰해야 합니다. 명령 이름 하나가 개발 서버, 테스트, 배포의 공통 입구를 바꿀 수 있기 때문입니다.
