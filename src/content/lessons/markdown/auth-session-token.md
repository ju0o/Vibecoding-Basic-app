...
### 설명 연습: 로그인 이후를 세 단계로 나누기

인증을 처음 설명할 때는 "로그인한다"라는 한 단어를 세 단계로 쪼개면 이해가 빨라집니다. 첫째, 사용자가 누구인지 확인합니다. 둘째, 서버가 그 확인 결과를 세션이나 토큰 형태로 기억하게 합니다. 셋째, 이후 요청마다 그 증표를 보내 사용자가 같은 사람임을 다시 확인합니다. 이 세 단계를 구분하면 비밀번호 입력 화면과 API 요청 헤더가 같은 흐름 안에 있다는 점이 보입니다.

또 하나 중요한 설명은 저장 위치입니다. 세션 쿠키는 브라우저가 자동으로 보내기 때문에 편리하지만 CSRF 같은 위험을 함께 봐야 하고, Authorization 토큰은 명시적으로 붙이기 때문에 API 클라이언트에 적합하지만 저장 위치와 만료 처리를 신경 써야 합니다. 결국 인증 설계는 "어떤 문자열을 쓰나"보다 "어디에 저장하고 언제 폐기하며 누가 읽을 수 있나"를 묻는 작업입니다.
